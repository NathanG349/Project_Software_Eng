...
export default function TripPage() {
  ...
  const handleSaveExpense = async () => {
    ...
    if (expenseForm.beneficiaries.length === 0) { alert("Select at least one person!"); return; }
            ...
            <div className="flex items-center gap-2">
              ...
              <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                ...
                <DialogContent className="sm:max-w-[500px] bg-white">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !editForm.startDate && "text-muted-foreground")}>
                              ...
                              {editForm.startDate ? format(editForm.startDate, "PPP", { locale: enUS }) : <span>Start Date</span>}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0 bg-white" align="start">
                            <Calendar mode="single" selected={editForm.startDate} onSelect={(date) => setEditForm({ ...editForm, startDate: date })} initialFocus locale={enUS} />
                          </PopoverContent>
                        </Popover>
                      </div>
                      <div className="grid gap-2">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !editForm.endDate && "text-muted-foreground")}>
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {editForm.endDate ? format(editForm.endDate, "PPP", { locale: enUS }) : <span>End Date</span>}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0 bg-white" align="start">
                            <Calendar mode="single" selected={editForm.endDate} onSelect={(date) => setEditForm({ ...editForm, endDate: date })} initialFocus locale={enUS} disabled={{ before: editForm.startDate }} />
                          </PopoverContent>
                        </Popover>
                      </div>
                    </div>
                  ...
                  <DialogFooter>
                    <Button onClick={handleUpdateTrip} className="bg-blue-600 hover:bg-blue-700 text-white">Save</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
          ...
          <Card className="shadow-sm border-blue-100 h-fit">
            <CardHeader className="bg-blue-50/50 border-b pb-4 flex flex-row justify-between items-center">
              ...
              <Dialog open={isActivityModalOpen} onOpenChange={setIsActivityModalOpen}>
                <Button size="sm" onClick={() => openActivityModal(null)} className="bg-blue-600 hover:bg-blue-700 text-white">➕ Add</Button>
                ...
                <DialogContent className="sm:max-w-[425px] bg-white">
                  ...
                  <div className="grid gap-4 py-4">
                    <div className="grid gap-2"><Label>Name</Label><Input placeholder="Ex: Restaurant" value={activityForm.name} onChange={e => setActivityForm({ ...activityForm, name: e.target.value })} /></div>
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !activityForm.date && "text-muted-foreground")}>
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {activityForm.date ? format(activityForm.date, "PPP", { locale: enUS }) : <span>Pick a date</span>}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0 bg-white" align="start">
                          <Calendar mode="single" selected={activityForm.date} onSelect={(date) => setActivityForm({ ...activityForm, date: date })} initialFocus locale={enUS} defaultMonth={activityForm.date || (trip.startDate ? new Date(trip.startDate) : new Date())} disabled={[trip.startDate ? { before: new Date(trip.startDate) } : null, trip.endDate ? { after: new Date(trip.endDate) } : null]} />
                        </PopoverContent>
                      </Popover>
                    </div>
                    ...
                    <div className="grid gap-2"><Label>Address</Label><Input placeholder="12 Street..." value={activityForm.address} onChange={e => setActivityForm({ ...activityForm, address: e.target.value })} /></div>
          ...
          <Card className="shadow-sm border-orange-100 h-fit">
            <CardHeader className="bg-orange-50/50 border-b pb-4 flex flex-row justify-between items-center">
              ...
              <Dialog open={isExpenseModalOpen} onOpenChange={setIsExpenseModalOpen}>
                <Button size="sm" onClick={() => openExpenseModal(null)} className="bg-orange-600 hover:bg-orange-700 text-white">➕ Add</Button>
                <DialogContent className="sm:max-w-[500px] bg-white">
                  <DialogHeader><DialogTitle>{editingExpenseId ? 'Edit Expense' : 'Add Expense'}</DialogTitle></DialogHeader>
                  <div className="grid gap-6 py-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="grid gap-2"><Label>What?</Label><Input placeholder="Ex: Beer" value={expenseForm.title} onChange={e => setExpenseForm({ ...expenseForm, title: e.target.value })} /></div>
                      <div className="grid gap-2"><Label>Amount (€)?</Label><Input type="number" placeholder="0" value={expenseForm.amount} onChange={e => setExpenseForm({ ...expenseForm, amount: e.target.value })} /></div>
                    </div>
                    <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      <div className="flex justify-between items-end">
                        ...
                        <button onClick={handleToggleSelectAll} className="text-xs text-orange-600 font-bold hover:underline">{isAllSelected ? "Unselect All" : "Select All"}</button>
                      </div>
                  ...
                  <DialogFooter><Button onClick={handleSaveExpense} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold">{editingExpenseId ? 'UPDATE' : 'ADD'}</Button></DialogFooter>
                </DialogContent>
              </Dialog>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
                ...
                {expenses.map(exp => (
                  <li key={exp._id} className="group flex justify-between items-center bg-white p-3 rounded-lg border shadow-sm hover:border-orange-200 transition-colors">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">Paid by <span className="font-medium text-gray-700">{exp.payer}</span></div>
                    </div>
              ...
              {balance && (
                <div className="mt-4 bg-slate-900 text-white p-5 rounded-xl shadow-xl">
                  <h3 className="font-bold mb-4 border-b border-gray-700 pb-2 text-lg">Reimbursements:</h3>
                  <ul className="space-y-3 text-sm">
                    {balance.reimbursements.map((remb, i) => (
                      <li key={i} className="flex items-center justify-between bg-slate-800 p-3 rounded-lg">
                        <div className="flex items-center gap-2">🔴 <span className="font-bold text-red-300">{remb.from}</span> <span className="text-gray-400">owes</span></div>
                        <span className="bg-white text-black px-2 py-1 rounded font-bold text-base">{remb.amount}€</span>
                        <div className="flex items-center gap-2"><span className="text-gray-400">to</span> 🟢 <span className="font-bold text-green-300">{remb.to}</span></div>
                      </li>
                    ))}
                    {balance.reimbursements.length === 0 && <li className="text-green-400 font-bold text-center text-lg">✅ Everything is balanced!</li>}
                  </ul>
                </div>
              )}
            </CardContent>
